...
func (r *RedisFailoverService) UpdateRedisFailoverStatus(ctx context.Context, namespace string, redisFailover *redisfailoverv1.RedisFailover, opts metav1.UpdateOptions) {
	_, err := r.k8sCli.DatabasesV1().RedisFailovers(namespace).Update(ctx, redisFailover, opts)
	if err != nil {
		recordMetrics(namespace, "RedisFailover", metrics.NOT_APPLICABLE, "UPDATE", err, r.metricsRecorder)
		r.logger.Errorf("Error while updating RedisFailover %s/%s : %s", redisFailover.Namespace, redisFailover.Name, err.Error())
	}
}
